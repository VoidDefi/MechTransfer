...
namespace MechTransfer.Items
{
    ...
    {
        public override string Texture => (GetType().Namespace + ".OldFilters." + Name).Replace('.', '/');
        ...
        public int DrawItemCategoryType = -1;
        ...
        private void Draw(SpriteBatch spriteBatch, Vector2 position, Color drawColor, Vector2 origin, float scale, float rotation)
        {
            ...
            if (DrawItemCategoryType >= 0)
            {

                if (!TextureAssets.Item[DrawItemCategoryType].IsLoaded)
                {
                    ModContent.Request<Texture2D>($"Terraria/Images/Item_{DrawItemCategoryType}", AssetRequestMode.ImmediateLoad); //Load Item Texture in Terraria
                }

                Texture2D categoryTexture = TextureAssets.Item[DrawItemCategoryType].Value;
